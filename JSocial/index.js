...
app.get('/main',  (req,res,next) => {
    ...
    jwt.verify(token,'supersecret',(error,decoded) => {
        if(!error){
            const html = search.readFile('./frontend/chat_page/public/chatpage.html')
            .then( data => {
                  ...
                  if(search.findFolder(`./backend/users/user_${id}`)){
                     ...
                     search.readdir(`./backend/users/user_${id}/uploads`)
                     .then(item => {
                        if(item) {
                            $('#user_pic_zoom').attr('src',`./${item}`)
                            $('#user_pic').attr('src',`./${item}`)
                            $('#initialpic').attr('src',`./${item}`)
                        }
                     })
                     
                     //enviando o HTML para o usuário
                     search.readFile(`./backend/users/user_${id}/infos.json`)
                     .then( data => {
                        $('#iduser').text(id)
                        $('#gid').attr("idnumber",id)
                        setTimeout(() => {if($('#nameipt').val() == 'indefinido') search.writeInFile(`./backend/users/user_${id}/infos.json`,`{"name":"${$('#nameipt').val()}"}`)},1)
                        
                        if(data){
                            $('#nameipt').val(JSON.parse(data).name)
