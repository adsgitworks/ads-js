...
app.use(express.static(path.join(__dirname,'frontend/login_page/assets')))
app.use(express.static(path.join(__dirname,'frontend/chat_page/assets/images')))
app.use(express.static(path.join(__dirname,'frontend/chat_page/assets/js')))
app.use(express.static(path.join(__dirname,'frontend/login_page/assets')))
...
app.post('/friends', async (req,res,next) => {
    if(req.body.requestedId){
        ...
        let data = JSON.parse(await search.readFile(`./backend/users/user_${id}/solicitacoes.json`))
    ...
    if(req.body){
        ...
        search.objectKeys(data,async function(obj){
            if(req.body.from == obj.from) {
                obj.accepted = true
                obj.utilized = false
                from[from.length] = {to: obj.from, from: obj.to, accepted: true, utilized: false}
            ...
            if(from_infos) from_infos[Date.now()] = from[0]
...
socket_io.on('connection', function(socket) {
        ...
        socket.on('sendReceivedToServer', function (messages) {
            ...
            function storageMessages(iduser,reverse){
                search.readFile(`./backend/users/user_${iduser}/messages.json`).then(data => {
                    ...
                    if(allmsgs) {
                        var ver = (messages['from'] ?  messages['from'] : messages.to)
    ...
    //socket.on('waitingNewFriendRequest', async function(USERID){
